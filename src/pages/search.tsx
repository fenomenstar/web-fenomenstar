import { useState, useEffect } from "react";
import { Layout } from "@/components/ui/Layout";
import { useSearch, SearchType } from "@workspace/api-client-react";
import {
  Search as SearchIcon,
  User,
  Play,
  Trophy,
  Sparkles,
  Brain,
  TrendingUp,
  X,
  ChevronRight,
} from "lucide-react";
import { Link } from "wouter";
import { motion } from "framer-motion";
import { cn } from "@/lib/utils";

const TRENDING = ["Tarkan", "Dans", "Karaoke", "Ses Yarışması", "THY", "Türkü", "Hip-Hop", "Akustik"];

export default function SearchPage() {
  const [query, setQuery] = useState("");
  const [debouncedQuery, setDebouncedQuery] = useState("");
  const [type, setType] = useState<SearchType>("all");
  const [aiMode, setAiMode] = useState(true);

  useEffect(() => {
    const handle = setTimeout(() => setDebouncedQuery(query), 350);
    return () => clearTimeout(handle);
  }, [query]);

  const { data, isLoading } = useSearch(
    {
      q: debouncedQuery,
      type,
      mode: aiMode ? "hybrid" : "fts",
      limit: 20,
    },
    { query: { enabled: debouncedQuery.length >= 2 } },
  );

  const hasResults =
    (data?.users?.length ?? 0) > 0 ||
    (data?.videos?.length ?? 0) > 0 ||
    (data?.competitions?.length ?? 0) > 0;

  const tabs = [
    { id: "all", label: "Tümü", icon: <Sparkles className="w-4 h-4" /> },
    { id: "users", label: "Kullanıcılar", icon: <User className="w-4 h-4" /> },
    { id: "videos", label: "Videolar", icon: <Play className="w-4 h-4" /> },
    { id: "competitions", label: "Yarışmalar", icon: <Trophy className="w-4 h-4" /> },
  ];

  return (
    <Layout>
      <div className="max-w-4xl mx-auto p-4 md:p-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-display font-black">Keşfet ve Ara</h1>
            <p className="text-muted-foreground text-sm mt-0.5">
              Kullanıcı, video, marka ve yarışmaları bul.
            </p>
          </div>
          <button
            onClick={() => setAiMode((value) => !value)}
            className={cn(
              "flex items-center gap-2 px-4 py-2.5 rounded-full border text-sm font-bold transition-all",
              aiMode
                ? "bg-gradient-to-r from-violet-600 to-purple-500 border-transparent text-white shadow-lg shadow-purple-500/30"
                : "bg-white/5 border-white/10 text-muted-foreground hover:border-primary/40",
            )}
          >
            <Brain className="w-4 h-4" />
            {aiMode ? "Akıllı Arama Açık" : "Klasik Arama"}
          </button>
        </div>

        <div className="relative mb-4">
          <input
            type="text"
            placeholder={
              aiMode
                ? "Akıllı arama ile keşfet..."
                : "Kullanıcı, video veya yarışma ara..."
            }
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            className="w-full bg-card border-2 border-white/10 rounded-2xl py-5 pl-14 pr-12 text-lg text-white focus:outline-none focus:border-primary transition-colors shadow-lg placeholder:text-muted-foreground"
          />
          <SearchIcon className="absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 text-muted-foreground" />
          {query && (
            <button
              onClick={() => setQuery("")}
              className="absolute right-5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-white transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          )}
        </div>

        {!query && (
          <div className="mb-8">
            <p className="text-xs text-muted-foreground uppercase tracking-widest mb-3 font-semibold flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-primary" />
              Trend Aramalar
            </p>
            <div className="flex flex-wrap gap-2">
              {TRENDING.map((item) => (
                <button
                  key={item}
                  onClick={() => setQuery(item)}
                  className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-sm text-gray-300 hover:bg-primary/20 hover:border-primary/40 hover:text-white transition-all"
                >
                  🔥 {item}
                </button>
              ))}
            </div>
          </div>
        )}

        <div className="flex gap-2 overflow-x-auto no-scrollbar mb-8">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setType(tab.id as SearchType)}
              className={cn(
                "flex items-center gap-2 px-5 py-2.5 rounded-full font-medium whitespace-nowrap transition-all text-sm",
                type === tab.id
                  ? "bg-primary text-white shadow-[0_0_15px_rgba(255,0,255,0.3)]"
                  : "bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white",
              )}
            >
              {tab.icon}
              {tab.label}
            </button>
          ))}
        </div>

        {isLoading ? (
          <div className="space-y-4">
            {[1, 2, 3, 4].map((item) => (
              <div key={item} className="h-24 bg-white/5 rounded-2xl animate-pulse" />
            ))}
          </div>
        ) : !hasResults && debouncedQuery ? (
          <div className="text-center py-20 text-muted-foreground">
            <SearchIcon className="w-14 h-14 mx-auto mb-4 opacity-30" />
            <p className="text-lg font-medium">"{debouncedQuery}" için sonuç bulunamadı.</p>
          </div>
        ) : (
          <div className="space-y-12">
            {aiMode && hasResults && (
              <div className="flex items-center gap-2 text-xs text-violet-400">
                <Brain className="w-3.5 h-3.5" />
                <span>Sonuçlar akıllı arama ile sıralandı.</span>
              </div>
            )}

            {(type === "all" || type === "users") && (data?.users?.length ?? 0) > 0 && (
              <section>
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-muted-foreground uppercase tracking-wider text-xs">
                  <User className="w-4 h-4 text-primary" />
                  Kullanıcılar
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {data!.users!.map((user: any, index: number) => (
                    <motion.div
                      key={user.id}
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: index * 0.05 }}
                    >
                      <Link
                        href={`/profile/${user.id}`}
                        className="flex items-center gap-4 p-4 bg-card rounded-2xl border border-white/5 hover:border-primary/30 hover:bg-white/5 transition-all group"
                      >
                        {user.avatarUrl ? (
                          <img
                            src={user.avatarUrl}
                            className="w-14 h-14 rounded-full object-cover border-2 border-white/10 group-hover:border-primary/40 transition-colors"
                            alt={user.username}
                          />
                        ) : (
                          <div className="w-14 h-14 rounded-full border-2 border-white/10 flex items-center justify-center font-black">
                            {(user.displayName || user.username).charAt(0).toUpperCase()}
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-base group-hover:text-primary transition-colors">
                            @{user.username}
                          </p>
                          <p className="text-sm text-muted-foreground truncate">
                            {user.displayName}
                          </p>
                        </div>
                        <span className="text-xs bg-white/5 px-3 py-1 rounded-full text-muted-foreground">
                          {user.city || "TR"}
                        </span>
                      </Link>
                    </motion.div>
                  ))}
                </div>
              </section>
            )}

            {(type === "all" || type === "videos") && (data?.videos?.length ?? 0) > 0 && (
              <section>
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-muted-foreground uppercase tracking-wider text-xs">
                  <Play className="w-4 h-4 text-secondary" />
                  Videolar
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {data!.videos!.map((video: any, index: number) => (
                    <motion.div
                      key={video.id}
                      initial={{ opacity: 0, scale: 0.95 }}
                      animate={{ opacity: 1, scale: 1 }}
                      transition={{ delay: index * 0.05 }}
                    >
                      <Link href={`/feed?videoId=${video.id}`} className="relative aspect-[3/4] rounded-xl overflow-hidden group block">
                        <img
                          src={video.thumbnailUrl}
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                          alt={video.title}
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                        <div className="absolute bottom-0 left-0 right-0 p-3">
                          <p className="text-xs font-bold text-white line-clamp-2">{video.title}</p>
                          <p className="text-[10px] text-gray-400 mt-1">@{video.username}</p>
                        </div>
                      </Link>
                    </motion.div>
                  ))}
                </div>
              </section>
            )}

            {(type === "all" || type === "competitions") &&
              (data?.competitions?.length ?? 0) > 0 && (
                <section>
                  <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-muted-foreground uppercase tracking-wider text-xs">
                    <Trophy className="w-4 h-4 text-yellow-400" />
                    Yarışmalar
                  </h3>
                  <div className="space-y-3">
                    {data!.competitions!.map((competition: any, index: number) => (
                      <motion.div
                        key={competition.id}
                        initial={{ opacity: 0, x: -10 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: index * 0.05 }}
                      >
                        <Link
                          href={`/competitions/${competition.id}`}
                          className="flex items-center gap-4 p-4 bg-card rounded-2xl border border-white/5 hover:border-yellow-400/30 hover:bg-white/5 transition-all group"
                        >
                          <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 bg-gradient-to-br from-indigo-900 to-purple-900">
                            <img
                              src={competition.thumbnailUrl}
                              className="w-full h-full object-cover"
                              alt={competition.title}
                            />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="font-bold group-hover:text-yellow-400 transition-colors line-clamp-1">
                              {competition.title}
                            </p>
                            <p className="text-xs text-muted-foreground mt-0.5">
                              {competition.category} · {competition.participantCount ?? 0} katılımcı
                            </p>
                          </div>
                          <span
                            className={cn(
                              "text-[10px] font-black px-2.5 py-1 rounded-full shrink-0",
                              competition.status === "active"
                                ? "bg-emerald-500/20 text-emerald-400"
                                : "bg-gray-500/20 text-gray-400",
                            )}
                          >
                            {competition.status === "active" ? "AKTİF" : "KAPALI"}
                          </span>
                        </Link>
                      </motion.div>
                    ))}
                  </div>
                </section>
              )}
          </div>
        )}

        {debouncedQuery.length > 0 && data && (
          <div className="mt-8 text-xs text-muted-foreground flex items-center gap-2">
            <ChevronRight className="w-3 h-3" />
            Toplam {data.total} sonuç bulundu
          </div>
        )}
      </div>
    </Layout>
  );
}
